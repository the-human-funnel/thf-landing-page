...
export async function POST(request: Request) {
  // Initialize Resend with API key (only at runtime, not build time)
  const apiKey = process.env.RESEND_API_KEY

  if (!apiKey) {
    console.error("RESEND_API_KEY is not configured")
    return NextResponse.json(
      { error: "Email service is not configured. Please contact us directly at adityaghailbdrp1@gmail.com" },
      { status: 500 }
    )
  }
